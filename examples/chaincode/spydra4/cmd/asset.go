...
func (s *SmartContract) GetAssetByQueryString(ctx contractapi.TransactionContextInterface, queryString string) (string, error) {
	...
	if err != nil {
		logger.Errorf("Error in fetching data from world state - %s", err.Error())
		return "", err
	}
	defer resultsIterator.Close()
	...
	for resultsIterator.HasNext() {
		queryResponse, err := resultsIterator.Next()
		if err != nil {
			logger.Errorf("Error iterating data - %s", err.Error())
			return "", err
		}
		if bArrayMemberAlreadyWritten {
			// Add a comma before array members, suppress it for the first array member
...
func (s *SmartContract) CreateAsset(ctx contractapi.TransactionContextInterface, assetData string) (string, error) {
	...
	if len(assetData) == 0 {
		logger.Errorf("Invalid doc data")
	...
	if err != nil {
		logger.Errorf("Failed while un-marshling document. %s", err.Error())
		return "", fmt.Errorf("failed while un-marshling document. %s", err.Error())
	}

	eventKey = asset.Type + asset.ID
...
func (s *SmartContract) GetAssetById(ctx contractapi.TransactionContextInterface, assetId string) (string, error) {
	...
	if len(assetId) == 0 {
		logger.Errorf("Please provide correct assets Id.")
	...
	if err != nil {
		logger.Errorf("Error fetching asset from world state. %s", err.Error())
		return "", fmt.Errorf("error fetching asset from world state. %s", err.Error())
	}

	if assetAsBytes == nil {
		logger.Errorf("%s does not exist", assetId)
...
func (s *SmartContract) CheckAssetListById(ctx contractapi.TransactionContextInterface, assetIdList string) (string, error) {
	...
	logger.Info("Assets list ", assetIdList)
	...
	logger.Info("Printing products struct array.")

	count := 0
	for i := range assetIds {
		eventKey := assetIds[i].AssetType + assetIds[i].AssetId
		// eventKey := assetIds[i].AssetType + assetIds[i].AssetId
		logger.Info("Checking asset available with name - ", eventKey)
		...
		logger.Info("Asset found." + string(assetAsBytes))

	}

	if count > 0 {
		logger.Errorf("Either Asset Id not found or Asset Type not matching for asset id - %s.", noAssets)
...
// 		if err != nil {
// 			logger.Errorf("Error iterating results - %s", err.Error())
// 			return "", fmt.Errorf(err.Error())
// 		}
// 		if bArrayMemberAlreadyWritten {
// 			buffer.WriteString(",")
